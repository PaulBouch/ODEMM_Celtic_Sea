...
graph_obj <- function(data, InSector, InPressure, InEco, method, percent){
  ...
  data$Colour = ifelse (data$key %in% dataselected$key, 
                        ifelse (data$Sector == 'Fishing',  paste0 (data$Links, " [penwidth = 20, color = red]; "),
                                ifelse (data$Sector == 'Aggregates',  paste0 (data$Links, " [penwidth = 20, color = coral]; "),
                                        ifelse (data$Sector == 'Agriculture',  paste0 (data$Links, " [penwidth = 20, color = cadetblue2]; "),
                                                ifelse (data$Sector == 'Aquaculture',  paste0 (data$Links, " [penwidth = 20, color = blue3]; "),
                                                        ifelse (data$Sector == 'Coastal Infrastructure',  paste0 (data$Links, " [penwidth = 20, color = saddlebrown]; "),
                                                                ifelse (data$Sector == 'Harvesting/Collecting',  paste0 (data$Links, " [penwidth = 20, color = gold1]; "), 
                                                                        ifelse (data$Sector == 'Land-based Industry',  paste0 (data$Links, " [penwidth = 20, color = chocolate2]; "),
                                                                                ifelse (data$Sector == 'Military',  paste0 (data$Links, " [penwidth = 20, color = gray50]; "),
                                                                                        ifelse (data$Sector == 'Navigational Dredging',  paste0 (data$Links, " [penwidth = 20, color = green4]; "),
                                                                                                ifelse (data$Sector == 'Non-renewable',  paste0 (data$Links, " [penwidth = 20, color = green]; "),
                                                                                                        ifelse (data$Sector == 'Renewable Energy',  paste0 (data$Links, " [penwidth = 20, color = aquamarine]; "),
                                                                                                                ifelse (data$Sector == 'Research',  paste0 (data$Links, " [penwidth = 20, color = midnightblue]; "),
                                                                                                                        ifelse (data$Sector == 'Shipping',  paste0 (data$Links, " [penwidth = 20, color = lightpink]; "),
                                                                                                                                ifelse (data$Sector == 'Telecommunications',  paste0 (data$Links, " [penwidth = 20, color = yellow]; "),
                                                                                                                                        ifelse (data$Sector == 'Tourism/Recreation',  paste0 (data$Links, " [penwidth = 20, color = springgreen]; "),
                                                                                                                                                ifelse (data$Sector == 'Waste Water',  paste0 (data$Links, " [penwidth = 20, color = plum3]; "),         
                                                                                                                                                        paste0 (data$Links, " [penwidth = 20, color = black]; "))))))))))))))))),
                        paste0 (data$Links, " [penwidth = 2, color = grey];")) 
